...
#include <pcl/visualization/registration_visualizer2.h>
    ...
    Eigen::Matrix4f &final_transform, bool downsample = false)
{
  ...
  //
  // Align
  pcl::MyICPNonLinear<PointNormalT, PointNormalT> reg;
...
//  pcl::CorrespondencesPtr correspondences_ptr (new pcl::Correspondences);
  ...
  for (int i = 0; i < 30; ++i)
  {
    ...
    pcl::CorrespondencesPtr correspondences_ptr = reg.getCorrespondances ();
    printf ("numCorrespondances=%d\n", correspondences_ptr->size ());
    if (correspondences_ptr->size () > 0)
    {
      pcl::RegistrationVisualizer2<PointNormalT, PointNormalT> vis;
      ...
      vis.startDisplay ();
      vis.setMaximumDisplayedCorrespondences(1000);
      vis.updateIntermediateCloud (*points_with_normals_src, indicesSrc, *points_with_normals_tgt, indicesTarget);
      getchar ();
      vis.stopDisplay ();
    }
